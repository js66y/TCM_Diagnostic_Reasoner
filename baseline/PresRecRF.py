...
path = 'C:\\Project\\JST\\TCM\\other\\PresRecRF'
...
class TCM_Model(torch.nn.Module):
    def __init__(self, batch_size, embedding_dim, symptom_cnt, herb_cnt, drop_ratio, cat_cardinalities, semantic='BERT', device="cuda:1"):
        ...
        self.relu = torch.nn.ReLU()
        self.trans = torch.nn.Linear(1350, 1804)
        self.trans2 = torch.nn.Linear(410, 510)
    ...
    def forward(self, X, bank, gat):
        ...
        # # add sym s
        # print(bert_sym.shape)  # [1804, 64]
        # print(self.sym_embedding.weight.shape) # [1804, 256]
        ...
        herb_emb = self.trans2(herb_emb.T)
        # 4. judge herb
        judge_herb = torch.mm(sym_agg, herb_emb)  # 64*128 * 128*410 => 64*410
        # 5. dosage
        judge_dosage = self.mlp_dosage(judge_herb)
        judge_dosage = self.mlp_dosage(self.relu(judge_dosage))
        outputs = {
            'pred_logits': judge_herb,
            'pred_values': judge_dosage
        }
        return outputs